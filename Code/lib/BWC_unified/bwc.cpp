...
void BWC::loop() {
    ...
    if (dsp->dsp_toggles.unit_change) {
        cio->cio_states.unit
            ? cio->cio_toggles.target = C2F(cio->cio_toggles.target)
            : cio->cio_toggles.target = F2C(cio->cio_toggles.target);
    }

    /*following method will change target temp and set _dsp_tgt_used to false if
     * target temp is changed*/
    _handleCommandQ();
...
void BWC::play_sound() {
    ...
    if (dsp->dsp_toggles.bubbles_change || dsp->dsp_toggles.heat_change ||
...
bool BWC::_compare_command(const command_que_item& i1,
...
void BWC::_handleNotification() {
    ...
    /* not the time yet*/
    if ((int64_t)_command_que[0].xtime - (int64_t)_timestamp_secs >
        (int64_t)_next_notification_time)
        return;
    /* only _notify for these commands*/
    if (!(_command_que[0].cmd == SETBUBBLES ||
          _command_que[0].cmd == SETHEATER || _command_que[0].cmd == SETJETS ||
          _command_que[0].cmd == SETPUMP))
        return;
    ...
    if (_next_notification_time <= 2)
        _next_notification_time =
            -10;  // postpone "alarm" until after the command xtime (will be
...
void BWC::_handleCommandQ() {
    ...
    if (_command_que[0].interval > 0) {
        _command_que[0].xtime =
            (uint64_t)time(nullptr) + _command_que[0].interval;
        _command_que.push_back(_command_que[0]);
    }
    _handlecommand(_command_que[0].cmd, _command_que[0].val,
                   _command_que[0].text);
...
bool BWC::_handlecommand(Commands cmd, int64_t val, const String& txt = "") {
    ...
    switch (cmd) {
        case SETTARGET: {
            ...
            /*Send this value to cio instead of results from button presses on
             * the display*/
            _dsp_tgt_used = false;
        ...
        case SETGODMODE:
            ...
            break;
        case SETFULLPOWER:
            val = constrain((int)val, 0, 1);
            cio->cio_toggles.no_of_heater_elements_on = val + 1;
            break;
        /*PRINTTEXT is not a command per se. Every command prints the txt
         * string, and if we ONLY want to print txt we do nothing to the
         * command.*/
        case SETREADY: {
            command_que_item item;
            if ((int64_t)_timestamp_secs >
                (int64_t)(val - _estHeatingTime() * 3600.0f -
                          7200))  // 2 hours extra margin
            {
                ...
                /*We can't use addcommand() because it will copy xtime to val
                 * again*/
                _command_que.push_back(item);
                std::sort(_command_que.begin(), _command_que.end(),
                          _compare_command);
            }
        } break;
        case SETR:
...
void BWC::_handleStateChanges() {
    if (_prev_cio_states != cio->cio_states ||
        _prev_dsp_states.brightness != dsp->dsp_states.brightness)
    ...
    if (cio->cio_states.unit != _prev_cio_states.unit ||
    ...
    /* check changes from DSP 4W - go to antigodmode if someone presses a
     * button*/
}
...
// virtual temp is always C in this code and will be converted when sending
// externally
void BWC::_calcVirtualTemp() {
    ...
    if (cio->cio_states.pump &&
        ((millis() - _pump_change_timestamp_ms) > 5 * 60000)) {
...
void BWC::_updateVirtualTempFix_ontempchange() {
    ...
    // readings are only valid if pump is running and has been running for 5
    // min.
    if (!cio->cio_states.pump ||
        ((millis() - _pump_change_timestamp_ms) < 5 * 60000))
        return;
    ...
    1 degree/(temperature age in ms / 3600 / 1000)hours = 3 600 000 /
    temperature age in ms
    */

    // We can only know something about rate of change if we had continous
    // cooling since last update (Nobody messed with the heater during the 1
    // degree change)
    if (_heatred_change_timestamp_ms > _temp_change_timestamp_ms)
        return;  // bugfix by @cobaltfish
    // rate of heating is not subject to change (fixed wattage and pool size) so
    // do this only if cooling and do not calibrate if bubbles has been on
    if (_vt_calibrated)
        return;
    if (cio->cio_states.heatred || cio->cio_states.bubbles ||
        (_bubbles_change_timestamp_ms > _temp_change_timestamp_ms))
        return;
    ...
    if (abs(degAboveAmbient) <= 1)
        return;
    _R_COOLING = ((millis() - _temp_change_timestamp_ms) / 3600000.0) /
                 log((conversion * degAboveAmbient) /
                     (conversion * (degAboveAmbient + _deltatemp)));
    _vt_calibrated = true;
    _save_settings_needed = true;
}
...
bool BWC::add_command(command_que_item command_item) {
    ...
    if (command_item.cmd == SETREADY) {
        command_item.val =
            (int64_t)command_item
                .xtime;          // Use val field to store the time to be ready
        command_item.xtime = 0;  // And start checking now
        command_item.interval = 0;
    }
    // add parameters to _command_que[rows][parameter columns] and sort the
    // array on xtime.
...
bool BWC::getBtnSeqMatch() {
    if (_btn_sequence[0] == POWER && _btn_sequence[1] == LOCK &&
        _btn_sequence[2] == TIMER && _btn_sequence[3] == POWER) {
        return true;
    }
    return false;
}
...
void BWC::_updateTimes() {
    ...
    if (_override_dsp_brt_timer > 0)
        _override_dsp_brt_timer -=
            elapsedtime_ms;  // counts down to or below zero

    // watts, kWh today, total kWh
    float heatingEnergy = (_heatingtime + _heatingtime_ms / 1000) / 3600.0 *
                          cio->getPower().HEATERPOWER;
    float pumpEnergy =
        (_pumptime + _pumptime_ms / 1000) / 3600.0 * cio->getPower().PUMPPOWER;
    float airEnergy =
        (_airtime + _airtime_ms / 1000) / 3600.0 * cio->getPower().AIRPOWER;
    float idleEnergy =
        (_uptime + _uptime_ms / 1000) / 3600.0 * cio->getPower().IDLEPOWER;
    float jetEnergy =
        (_jettime + _jettime_ms / 1000) / 3600.0 * cio->getPower().JETPOWER;
    _energy_total_kWh =
        (heatingEnergy + pumpEnergy + airEnergy + idleEnergy + jetEnergy) /
        1000;  // Wh -> kWh
...
// // ESP.wdtDisable();
    ...
    doc[F("TGT")] = cio->cio_states.target;
    doc[F("GOD")] =
        (uint8_t)cio->cio_states.godmode;  // makes the file look better
...
bool BWC::_load_melody_json(const String& filename) {
    ...
    */
    ...
    while (file.available()) {
        ...
        _notes.push_back(n);
        /*add a little break between the notes (will be placed before each note
         * due to reversing)*/
        n.frequency_hz = 0;
        n.duration_ms = beat_period * s.toFloat();
...
//     }
//     file.close();
//     /* We read and erase from the back of the vector (faster) so if notes are
//     stored in the natural order we need to reverse*/
//     std::reverse(_notes.begin(), _notes.end());
// }
